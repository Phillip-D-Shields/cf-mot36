<script>
    ...
    let { data, form } = $props(); // 'form' contains the result of our server action
    let quiz = $derived(data.quiz);
    let questions = $derived(data.questions || []);

    // State Management
    let step = $state('intro'); // 'intro' -> 'quiz' -> 'submitting' -> 'result'
    let volunteerName = $state('');
    let brigadeId = $state('');
    
    // Object to store answers: { "question_123": "opt_a", "question_124": "true" }
    ...
    // React to the server response. If form data arrives and is successful, show results.
    $effect(() => {
        if (form?.success) {
            step = 'result';
        }
...
<div class="container py-5 max-w-md" style="max-width: 800px; margin: 0 auto;">
    ...
    {:else if step === 'quiz' || step === 'submitting'}
        <div class="mb-4">
            ...
            <p class="text-muted">Volunteer: <strong>{volunteerName}</strong> {brigadeId ? `(#${brigadeId})` : ''}</p>
        </div>
        ...
        <form method="POST" action="?/submit" use:enhance={() => {
                        ...
                        <div class="ms-3">
                            {#if q.type === 'true_false'}
                                <div class="form-check mb-2">
                                    <input class="form-check-input" type="radio" name={'q_'+q.id} id={'t_'+q.id} value="true" bind:group={userAnswers[q.id]} required>
                                    <label class="form-check-label fs-5" for={'t_'+q.id}>True</label>
                                </div>
                                <div class="form-check">
                                    <input class="form-check-input" type="radio" name={'q_'+q.id} id={'f_'+q.id} value="false" bind:group={userAnswers[q.id]} required>
                                    <label class="form-check-label fs-5" for={'f_'+q.id}>False</label>
                                </div>
                            {:else}
                                {#each q.options as opt}
                                    <div class="form-check mb-2">
                                        <input class="form-check-input" type="radio" name={'q_'+q.id} id={'opt_'+opt.id} value={opt.id} bind:group={userAnswers[q.id]} required>
            ...
            <div class="card shadow-sm mt-5">
                <div class="card-body text-center p-4">
                    ...
                    <button type="submit" class="btn btn-success btn-lg px-5" disabled={step === 'submitting'}>
                        {#if step === 'submitting'}
    ...
    {:else if step === 'result'}
        <div class="card shadow border-0 text-center py-5">
            <div class="card-body">
                ...
                <div class="display-4 fw-bold my-4 border py-3 mx-auto rounded bg-light" style="max-width: 200px;">
                    {form?.score}%
                </div>

                <p class="text-muted small mb-5">Your results have been securely recorded and sent to the training officer.</p>
