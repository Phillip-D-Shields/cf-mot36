<script>
    ...
    function startQuiz() {
        ...
        // Pre-fill arrays for multi-choice questions
        ...
        <form
            ...
            use:enhance={() => {
                ...
                return async ({ update, result }) => {
                    await update();
                    // Handle state routing safely based on server result
                    if (result.type === 'success') {
                        step = 'result';
                    } else {
                        // Revert step so user can see the error message and try again
                        step = 'quiz'; 
                    }
                };
            }}
                    ...
                    <div class="card-body p-4">
                        ...
                        <div class="ms-3">
                            ...
                            {:else if q.type === 'multi_choice'}
                                {#each q.options as opt}
                                    <div class="form-check mb-2">
                                        <input class="form-check-input" type="checkbox" name={'q_' + q.id} id={'opt_' + opt.id} value={opt.id} bind:group={userAnswers[q.id]} />
                                        <label class="form-check-label fs-5" for={'opt_' + opt.id}>{opt.text}</label>
                                    </div>
                                {/each}
                            {/if}
