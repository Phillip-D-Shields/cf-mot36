<script lang="ts">
    ...
    let formState = $state({
        ...
        is_active: initialQuiz.is_active !== undefined ? (initialQuiz.is_active === 1 || initialQuiz.is_active === true) : true,
        questions: initialQuestions.length ? initialQuestions : [] as Array<{
            id: string;
            text: string;
            type: string;
